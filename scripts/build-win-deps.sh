...
#
# THE PREFIX IS SHARED, BY THREE CONSUMERS AND ON PURPOSE. Both products in this
# repository build from it -- they draw with one graphics stack out of core/, so
# giving them two sysroots would only create a way for one product's Windows
# glyphs to differ from the other's -- and so does the sibling plug-in NAMp, whose
# own copy of this script installs to the same default root. Override it with
# $NAMP_WIN_DEPS_ROOT if that is not wanted. The sharing is also why the FreeType
# patch below tests for a project-NEUTRAL sentinel -- see the comment at that step.
set -euo pipefail

TRIPLE="${TRIPLE:-x86_64-w64-mingw32}"

# THREE NAMES FOR THE PREFIX, AND THE OLD TWO STILL WORK. This script was two scripts
# -- products/rations/ and products/rack/ each had a copy -- and each read its own
# variable. Neither is broken here: a shell or a note that already says
# RATIONS_WIN_DEPS_ROOT or NAMPRACK_WIN_DEPS_ROOT keeps working, and NAMP_WIN_DEPS_ROOT
# is the name to use from now on. It is not invented for this merge either: the sibling
# project NAMp's own copy of this script already reads exactly that name, so pointing
# the third consumer at this file needs no change on its side.
ROOT="${NAMP_WIN_DEPS_ROOT:-${NAMPRACK_WIN_DEPS_ROOT:-${RATIONS_WIN_DEPS_ROOT:-$HOME/third_party/win-deps}}}"
SYSROOT="${NAMP_WIN_SYSROOT:-${NAMPRACK_WIN_SYSROOT:-${RATIONS_WIN_SYSROOT:-$ROOT/sysroot}}}"

# The ASIO step's two overrides, resolved once here for the same reason and in the same
# shape. Resolved ONCE rather than at each use: a nested ${A:-$B} read at the point of
# use is evaluated under `set -u`, so the spelling that is not set is an error rather
# than a fallback the moment only the other one is given.
SKIP_ASIO="${NAMP_SKIP_ASIO:-${NAMPRACK_SKIP_ASIO:-0}}"
ASIO_SDK_DIR="${NAMP_ASIO_SDK_DIR:-${NAMPRACK_ASIO_SDK_DIR:-}}"
DL="$ROOT/dl"
BUILD="$ROOT/build"
JOBS="$(nproc)"
...
if ! done_stamp freetype; then
    ...
    #
    # THE SENTINEL MUST WORK IN BOTH DIRECTIONS, AND IT DID NOT. Two scripts
    # install into this one prefix and either may reach this step first: this
    # file, which both products in this repository now share, and the sibling
    # project NAMp's own copy of it.
    #
    # Whichever runs second must see the first one's work and skip. The two
    # tests are not the same test, which is the whole problem:
    #
    #   here   grep -q  "DLL_EXPORT removed"                       (a substring)
    #   NAMp   grep -qF "# NAMp: -DDLL_EXPORT removed, this is a static build"
    #
    # So NAMp recognises only its OWN marker. On a tree this script had patched
    # first, its test would find neither its marker nor the upstream
    # ft2_defines line -- already replaced -- and it would take its else branch
    # and die, on a prefix that is in fact correctly patched. Measured, not
    # reasoned about: of the four orderings, three worked and that one aborted.
    #
    # The fix is one string and it is entirely on this side. NAMp greps for a
    # SUBSTRING of a line, so a marker that begins with NAMp's exact text and
    # then says who actually wrote it satisfies both tests at once. All four
    # orderings now skip correctly. Do not shorten this marker or move the
    # suffix in front of it: everything up to "static build" is load-bearing
    # text belonging to the other script's test, not a sentence to edit.
    FT_MESON="$ROOT/$FREETYPE/meson.build"
    FT_PATCH_MARK="# NAMp: -DDLL_EXPORT removed, this is a static build (scripts/build-win-deps.sh)"
...
#
# NAMP_SKIP_ASIO=1 SKIPS THIS STEP, and the rest of the sysroot still builds.
...
if [ "$SKIP_ASIO" = "1" ]; then
    say "ASIO SDK"
    echo "warning: NAMP_SKIP_ASIO=1 - the ASIO SDK was NOT installed." >&2
    ...
    echo "warning: the latency depends on. Re-run without NAMP_SKIP_ASIO to fetch it." >&2
elif ! done_stamp asiosdk; then
    say "ASIO SDK"

    ASIO_SRC=""
    if [ -n "$ASIO_SDK_DIR" ]; then
        ASIO_SRC="$ASIO_SDK_DIR"
        [ -d "$ASIO_SRC" ] || die "NAMP_ASIO_SDK_DIR=$ASIO_SRC is not a directory"
        echo "using $ASIO_SRC (NAMP_ASIO_SDK_DIR)"
...
if [ -f "$SYSROOT/include/asiosdk/common/asio.h" ]; then
    ...
    printf '  %-12s %s\n' "asiosdk" "skipped (NAMP_SKIP_ASIO=1) - WASAPI-only standalone"
else
    printf '  %-12s %s\n' "asiosdk" "MISSING"
    MISSING=1
fi
