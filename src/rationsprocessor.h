...
private:
    ...
    // Mono in, stereo out. The amp is mono the whole way down — gate, models, tone stack,
    // cabinet — and becomes stereo at the very last step by writing the same sample to both
    // channels. There is nothing in this plug-in that could make the two differ.
    //
    // It is stereo anyway, unconditionally, because that is what the rack downstream expects of
    // it: the amp is the anchor of the chain and structurally the point where the chain becomes
    // stereo, so effects placed after it get two channels to work with whether or not the amp
    // itself had anything stereo to say. `outR` is still checked for null, because a host is
    // entitled to give a mono output bus and the SDK validator does exactly that.
