...
//
// Three pages, each on its own window: the amp head (a photographic faceplate, eight
// rotated-bitmap dials, five bat toggles and their LEDs, a bypass toggle, level meters on both
// edges, Input and Output dials under the meters, and two buttons — one for the cabinet page and
// one for the settings page), the cabinet (the cab, a blend dial drawn over the knob painted into
// the art, and two impulse-response loader rows), and the MIDI settings page. Every art load is
// checked with a flat-colour fallback, so a missing Resources directory degrades rather than
// crashes.
//
...
// are made to agree. A row added on either side must be added on the other; a mismatch would draw
// four rows over five bindings and silently lose one.
...
void RationsEditorView::drawStaticLayer(Canvas &c)
{
    switch (mPage) {
        case geo::Page::Head:
            ...
            c.drawString(geo::kTitleWord, geo::kFaceCX - c.stringWidth(geo::kTitleWord) * 0.5f,
                         geo::kTitleBaselineY);
            break;
        case geo::Page::Setup:
...
void RationsEditorView::compose(Canvas &c)
{
    switch (mPage) {
        case geo::Page::Head:
            ...
            break;
        case geo::Page::Setup:
            // TWO COLUMNS, ONE PAGE. Each is composed in its own coordinates and translated into
            // place, which is what lets every constant in geometry.h's cabinet block stay written
            // against that column's width and every constant in its settings block stay written
            // against 640 — the widths they were designed at. It is the same one-translate idiom
...
Rect RationsEditorView::scrollTrackRect() const
{
    ...
    // kScrollBarPageX, not kScrollBarX: the bar is chrome and lives in page coordinates, outside
    // the translate that puts the settings column in place.
    return Rect::fromLTRB(static_cast<float>(geo::kScrollBarPageX), top,
                          static_cast<float>(geo::kScrollBarPageX + geo::kScrollBarW), bottom);
}
...
{
    // The settings COLUMN's centre, not the page's: this function draws in the column's own
    // coordinates and the painter translates it into place. See compose().
    ...
    // already discarded (see midilearn.h) - so a player whose footswitch sends on channel 2 and
    // whose keyboard sends the same CC on channel 1 needs to know that before they find out by
    // playing.
...
void RationsEditorView::onMouseDown(int x, int y, int button)
{
    ...
    if (button == 3) {
        ...
        if (mPage == geo::Page::Setup && geo::inSetupSettingsColumn(fx) && !mBrowser.isOpen() &&
            mController) {
            const float cx = fx - static_cast<float>(geo::kSetupSetColX);
            const float cy = contentY(fy);
            for (int i = 0; i < geo::kLevelRowCount; ++i) {
                if (!levelRowRect(i).contains(cx, cy))
                    continue;
    ...
    switch (mPage) {
        ...
        case geo::Page::Setup:
            ...
            // refusal; the columns below it are page content and are asked in their own
            // coordinates. Nothing above the fixed header band is content at all, which is what
            // stops a click beside the back button reaching a row that has scrolled underneath it.
...
void RationsEditorView::onMouseWheel(int x, int y, int delta)
{
    ...
    if (mPage == geo::Page::Setup) {
        ...
        // work. This page's right column is rows: twelve of them, 592 units wide in a 640-unit
        // column, with 8-unit gaps. The pointer is almost always over one, so "scrolls unless over
        // a control" means "does not scroll", and the first test of it wound a channel trim to
        // -11.5 dB while trying to reach the output section.
        //
        ...
        if (!geo::inSetupSettingsColumn(fx)) {
            ...
            if (blendActive() && hitCircle(bx, cy, geo::kBlendCX, geo::kBlendCY, geo::kBlendHitR))
        ...
        if (calValueRect().contains(cx, cy)) {
...
{
    if (!mController || mPage != geo::Page::Setup || mController->armedMidiRow() < 0)
        return;
    if (--mMidiPollTicks > 0)
        return;
