...
// Writes <prefix>-head.png, <prefix>-cabinet.png and <prefix>-settings.png.
...
// drawHeadStripStatic draw it and in the same order. This render is what the Windows panel diff
// compares against, so a difference here is a false regression.
void renderHeadStrip(Canvas &c)
{
...
void renderHead(Canvas &c, ImageCache &images, SvgCache &icons)
{
    ...
    // The wordmark: the NAMp badge with the title word under it. The same two draws as
    ...
    drawCenteredText(c, Font::Title, geo::kTitleSize, geo::kTextColor, geo::kTitleWord,
                     geo::kFaceCX, geo::kTitleBaselineY);
...
{
    // The settings COLUMN's centre. This function draws in the column's own coordinates and
    // the caller translates it into place, exactly as the editor's compose() does.
    const float cx = static_cast<float>(geo::kSetupSetColCX);
    c.setColor(geo::kBgColor);
    c.fillRect(c.bounds());
...
// The setup page: both columns, each drawn in its own coordinates and translated into place.
//
// DELIBERATELY THE SAME TWO CALLS THE EDITOR MAKES, in the same order and with the same offsets,
// rather than a re-implementation of the page. What this tool audits is whether the geometry a
// column is given is enough for the art it holds; if it composed the page differently from
// compose() it would be auditing a layout nobody sees.
void renderSetup(Canvas &c, ImageCache &images, SvgCache &svgs)
{
    c.setColor(geo::kBgColor);
    c.fillRect(c.bounds());
    ...
    renderCabinet(c, images, svgs);
    cairo_restore(c.cr());

    cairo_save(c.cr());
    cairo_translate(c.cr(), geo::kSetupSetColX, 0.0);
    renderSettings(c, images, svgs);
    cairo_restore(c.cr());

    // The way back, ONCE, outside both columns. It is chrome — the editor draws it in
    // composeChrome, after the scroll translate is undone — and it belongs to the page rather than
    // to either column. Each column used to be a page of its own and drew its own; drawing both
    // would put two of them side by side.
...
// the two are different questions. The title word carries exactly one capital, so
...
bool auditHitBoxes()
{
    ...
    // Slim's target, with the editor's own 4 px slop. It is the only furniture left in this
    // corner — the settings button that used to sit to its right was folded into the page-button
    // row — so its neighbours are now the wordmark on one side and the output meter on the other,
    // and it is exactly the kind of box this audit exists for.
    boxes.push_back({"slim icon", geo::kSlimIconCX - geo::kSlimIconW / 2.0f - 4,
                     ...
                     geo::kSlimIconCY + geo::kSlimIconH / 2.0f + 4});
    // The strip's four bank columns are not clickable, but they are in the check as obstacles
    // for the same reason the meters below are: a control drawn over a capture name is a fault
    ...
    // The top band's right edge is the reflection of the utility row's left edge — the bypass
    // lamp's — through the faceplate centre, which is what makes the two clusters sit the same
    // distance from their own meter column. That is a relationship, not a coincidence, so it is
    // checked rather than trusted: the two meter columns are themselves symmetric about that
    // centre, so if this ever fails it is because someone moved a column or moved the furniture
    // that ends on this edge, not because the arithmetic drifted.
    //
    // Three different controls have carried this edge — a gear icon, then a labelled settings
    // button, now the Slim icon that moved right when that button was folded into the page-button
    // row. The symmetry outlived all three, which is why it is asserted against the EDGE rather
    // than against whichever control happens to end on it.
    const int mirrored = 2 * geo::kFaceCX - (geo::kBypassLedCX - geo::kTopLedR);
    if (geo::kTopBandInkRight != mirrored) {
        fprintf(stderr,
                "panelrender: the top band's furniture ends at x=%d but the bypass lamp's mirror "
                "is x=%d — the corner no longer sits off the output meter the way the utility row "
                "sits off the input meter\n",
                geo::kTopBandInkRight, mirrored);
        ++hits;
    }

    if (hits == 0)
        printf("hit boxes  %zu targets on the faceplate, none overlapping; the corner mirrors "
               "the utility row\n",
               boxes.size());
    return hits == 0;
}
...
bool auditText(FontStack &fonts)
{
    ...
    fits.push_back({"wordmark", Font::Title, geo::kTitleSize, geo::kTitleWord, titleRoom});
    ...
    // Settings page.
    // The strip. Two kinds of string sharing one column's width.
    ...
    fits.push_back({"level heading", Font::Title, geo::kSettingsHeadingSize, geo::kLevelHeading,
                    static_cast<float>(geo::kMidiRowW)});
    // kLevelRowCount, not kMidiLearnRowCount. The two are the same number today; they are
    // different questions — how many channels have a trim, against how long the learn table is —
    // and they were different numbers while the learn table carried footswitch rows as well.
    for (int i = 0; i < geo::kLevelRowCount; ++i)
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        // past its right edge by design. What the text must not reach is the right margin the
        // scrollbar lives in, so the budget runs from the text's own left edge to where the
        // settings rows end.
    ...
    int bad = 0;
    {
        ...
        const float titleInkTop =
            static_cast<float>(geo::kTitleBaselineY) - c.stringAscent(geo::kTitleWord);
        const float titleInkBottom =
            static_cast<float>(geo::kTitleBaselineY) + c.stringDescent(geo::kTitleWord);
...
int main(int argc, char **argv)
{
    ...
    // Both pages share one floor now — the window's width is shared with the rack below it, so a
    // page with a lower floor would disagree about the smallest legal window. Each page is still
    // rendered whole here, at its full height and with no scroll, because what this tool audits is
    // the ART — whether a legend fits the space geometry.h gives it — and that question is asked
    // of the page, not of the window showing part of it.
    ...
    const PageOut pages[] = {
        ...
        {"-setup.png", geo::Page::Setup, renderSetup},
    };
