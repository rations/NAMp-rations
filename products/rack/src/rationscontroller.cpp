...
tresult PLUGIN_API RationsController::initialize(FUnknown *context)
{
    ...
    // channel is learnable — see kMidiLearnRows.
    ...
    // contradicting the whole reason Normalized is the default. Found in the parent amp by its LV2
    // build, where the two stop being separable: an LV2 host initialises a control port FROM
    // lv2:default, so the wrong field there is not a reset-only case but what a fresh instance
    // comes up sounding like. This amp gets its own LV2 bundle, so the same trap is live here.
    ...
    // --- the unit, and the ORDER rule that comes with it ---------------------------------
    //
    // There is ONE unit now, so nothing here can get the order wrong. The rule below is written
    // down anyway, because it is a measured finding that cost five builds to isolate and it
    // becomes live again the moment anyone adds a second unit.
    //
    // THE MIDI UNIT MUST BE THE FIRST UNIT THIS CONTROLLER ADDS. That was found by bisecting five
    // builds against a real host after a footswitch that had worked for months stopped being seen
    // at all. The parent plug-in's pedalboard had added a second unit and added it FIRST, which
    // pushed the MIDI unit from index 0 to index 1; moving that one addUnit call back behind this
    // one — changing nothing else, not a parameter, not a flag, not an id — restored the
    // footswitch completely. Every other candidate was eliminated by its own build: the extra
    // parameters, their position in the declaration order, and the reported latency of the pedal
    // ahead of them were each removed on their own and each one still failed.
    //
    // What that means INSIDE the host is not verifiable from here and is not claimed. The SDK's
    // own MIDI-to-parameter converter resolves the event bus's unit by ID — it walks getUnitInfo
    // and compares against what getUnitByBus returned (public.sdk/source/vst/basewrapper/
    // basewrapper.cpp, getProgramListAndUnit) — and a host doing that is indifferent to the
    // order. A host that instead takes the first unit, or indexes by position, is not, and the
    // measurement says at least one real host is in the second group.
    //
    // So: if a second unit is ever added here, it goes AFTER this line.
    //
    ...
    if (version >= 5) {
        ...
        for (int32 i = 0; i < count; ++i) {
            ...
            if (!streamer.readDouble(value))
                return kResultFalse;
            // Read and discarded — this build has no pedalboard. The stream still has to be
            // consumed, because the EQ switch and Slim sit after this block; mirror of the
            // processor's own read.
            (void)value;
        }
    }
