...
tresult PLUGIN_API RationsProcessor::setupProcessing(ProcessSetup &setup)
{
    ...
    // Latency is the resampler's alone. The pedalboard used to add an unconditional contribution
    // here; with it gone the amp's only delay is the resampler's, which is reported once here and
    // never changes again for the life of this setup. At 48 kHz the resampler is a straight
    // call-through and this is zero.
...
void RationsProcessor::handleParameterChanges(IParameterChanges *changes)
{
    ...
    for (int32 i = 0; i < count; ++i) {
        ...
        if (id >= kMidiCcBaseId && id <= kMidiCcLastId) {
            ...
            // selecting Clean twice is selecting Clean; on a row that TOGGLES it is a footswitch
            // that works once and is then dead. Measured against the built bundle, not reasoned
            // about:
...
void RationsProcessor::midiTrigger(MidiMsg msg, int channel, int data1)
{
    ...
    for (int r = 0; r < kMidiLearnRowCount; ++r) {
        ...
        // What the row performs. Every row today is a channel row and stores its own step; the
        // table is generic over ParamID, so a row that did something else would branch here and
        // nowhere else - see midilearn.h.
        //
        // The value that is actually stored is what gets echoed, never target.value. For a Set
        // row the two are the same, but keeping the distinction is what makes adding a row that
        // reads its value back a change in one place rather than three.
...
tresult PLUGIN_API RationsProcessor::process(ProcessData &data)
{
    ...
    if (!outL)
        return kResultOk;
    // The second channel, when the host gave one. The amp is mono the whole way down and the two
    // channels are the same samples by construction - see applyDsp's tail, which is the one place
    // stereo happens.
    ...
    // mono bus. Two is what this plug-in produces; a host asking for more than that is asking for
    // a wider version of the same thing, and duplicating is the only honest answer.
...
void RationsProcessor::applyDsp(const float *in, float *outL, float *outR, int32 numSamples)
{
    ...
    // 7. double -> float with output gain and the ramped bypass mix, duplicated to two channels.
    //
    // THE AMP IS MONO ALL THE WAY DOWN. The gate, the models, the tone stack and the cabinet each
    // take one channel and give back one channel, and nothing between the input and this line has
    // anything stereo to say. Stereo happens here and only here, by writing the same sample
    // twice.
    //
    // It happens unconditionally rather than only when a host asks for two channels, because the
    // amp is the anchor of a chain and this is structurally the point where that chain becomes
    // stereo. Anything placed after the amp gets two channels to work in; anything placed before
    // it is mono and runs once. That split is the rack's, and the amp's job is only to be the
    // place it happens.
    //
    ...
    //
    // mDryBuf is mono — it is the plug-in's own input — so both channels fade back to the same
    // thing, which is what bypass should mean: the plug-in is out of the circuit and the signal
    // coming in was mono.
    ...
    for (int32 i = 0; i < numSamples; ++i) {
        ...
        const double dry = mDryBuf[static_cast<size_t>(i)];
        const double wet = finalBuf[i] * outputGain;
        outL[i] = static_cast<float>(wet + mix * (dry - wet));
        // Null when the host gave a mono output bus, which the SDK validator does. The two
        // channels are the same number by construction, so this is a copy and not a second
        // calculation.
        if (outR)
            outR[i] = outL[i];
    }
    ...
    if (version >= 5) {
        ...
        for (int32 i = 0; i < count; ++i) {
            ...
            if (!streamer.readDouble(value))
                return kResultFalse;
            // Read and DISCARDED. This build has no pedalboard, so there is nothing for these to
            // land in — but the stream has to be consumed either way, because the EQ switch and
            // Slim sit after this block and would otherwise be read at the wrong offset. That is
            // exactly the job the length prefix was added for; it just happens to be doing it for
            // a block that went to zero rather than one that grew.
            (void)value;
        }
    }
...
tresult PLUGIN_API RationsProcessor::getState(IBStream *state)
{
    ...
    //
    // This build writes a count of ZERO and no values. The slot stays because the EQ switch and
    // Slim sit after it: removing four bytes from the middle of the blob would move both and make
    // every project written by an earlier build unreadable, which is a far worse trade than a
    // vestigial zero. A reader on either side of the change needs no special case — it reads the
    // count and takes what follows, and the count happens to be nought.
