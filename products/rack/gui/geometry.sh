# Master geometry and palette for the editor art.
#
...
# The NAMp wordmark that sits above the title word on the head page. BADGE_W/H is the
...
# There is no badge asset for the title word. The NAMp badge above it is art
# (BADGE_W/H); the word itself is drawn as text in Michroma at run time, the way
# the author's other plug-in draws its own name.
#
# The pedal enclosures, their grid and the footswitch cap were described here and
# built by make_pedals.sh. Both went with the pedalboard: the pedals are separate
# plug-ins now and each carries its own art, in its own project.
