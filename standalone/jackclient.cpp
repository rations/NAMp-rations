// JackClient implementation. See jackclient.h for what is JACK-specific and audiobackend.h for
                      ...
                      Vst::IComponent *component, const MidiRoute *route)
{
    ...
    if (!mClient) {
        fprintf(stderr, "namp-rack: cannot connect to JACK (is jackd running?)\n");
        return false;
    }
    ...
    if (!mInPort || !mOutPorts[0] || !mOutPorts[1]) {
        fprintf(stderr, "namp-rack: cannot register JACK ports\n");
        close();
        return false;
    }
    ...
    if (!mMidiPort)
        fprintf(stderr, "namp-rack: cannot register the MIDI port - no footswitch\n");
    ...
    if (!mProcessData.prepare(*component, 0, Vst::kSample32)) {
        fprintf(stderr, "namp-rack: cannot prepare the process buffers\n");
        close();
        return false;
    }
    ...
    if (jack_set_process_callback(mClient, processTrampoline, this) != 0) {
        fprintf(stderr, "namp-rack: cannot install the JACK process callback\n");
        close();
        return false;
    }
    ...
    if (jack_set_buffer_size_callback(mClient, bufferSizeTrampoline, this) != 0)
        fprintf(stderr, "namp-rack: cannot install the JACK buffer-size callback - the "
                        "processor will stay set up for the size it started with\n");

    if (jack_activate(mClient) != 0) {
        fprintf(stderr, "namp-rack: cannot activate the JACK client\n");
        close();
        return false;
    }
...
//------------------------------------------------------------------------
// JACK's notification thread. Counting only - anything more would be work on a thread whose job is
// to return immediately.
int JackClient::xrunTrampoline(void *arg)
{
    static_cast<JackClient *>(arg)->mXruns.fetch_add(1, std::memory_order_relaxed);
    return 0;
}
...
int JackClient::process(jack_nframes_t nframes)
{
    ...
    while (done < nframes) {
        ...
        // events would stomp the footswitch once per chunk, which on a channel that toggles is the
        // difference between one channel and another.
        mInputChanges.clearQueue();
        mOutputChanges.clearQueue();
        mEvents.clear();
        mProcessData.inputEvents = nullptr;
