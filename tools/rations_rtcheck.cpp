...
int main(int argc, char **argv)
{
    ...
    // the natural expression the next time a row type is added.
    {
        ...
        // Matching is only half of it. What a matched row PERFORMS is on the same thread, and it
        // is counted here in the same shape midiTrigger runs it: a channel row stores one atomic.
        // The learn table is generic over ParamID, so a later row type whose action wanted a map,
        // a string or a value read back before writing is caught here rather than in somebody's
        // session.
