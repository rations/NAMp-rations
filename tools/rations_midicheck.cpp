...
bool readTrims(MemoryStream &s, double out[kChannelCount], int32 *slotCount = nullptr,
               double *eqOn = nullptr, double *slim = nullptr)
{
    ...
    // between - including the length-prefixed slot below, one field further on.
    if (eqOn)
        *eqOn = 1.0; // what a version 1-6 project opens as
    if (slim)
        *slim = 1.0; // and what a version 1-7 project opens as: the whole model
    if (!slotCount && !eqOn && !slim)
        return true;

    // On to the length-prefixed slot, which means reading past the output section and the four
    // capture sources first. This is where the MIDI block's length prefix is really tested: get
    // that count wrong and everything from here on is read at the wrong offset, so a wrong answer
    // here is a loud failure rather than four plausible numbers.
    if (slotCount)
        *slotCount = 0;
    if (version < 4)
        return true;
...
// blob with four rows (what every project written before the footswitch rows looks like)
...
int main(int argc, char **argv)
{
    ...
    // --- 7b. the length prefixes, and what an old blob reads as -------------------------------
    //
    ...
    // the capture paths, the length-prefixed slot - at the wrong offset. That is why state
    // version 6 gave the block a count, and it is the only thing version 6 does.
    //
    // The SECOND prefix is checked here too, and it matters more now than when it was written.
    // The slot it measures held the pedalboard; this build has no pedals and writes a count of
    // ZERO into it, and the EQ switch and Slim still sit on the far side. So a zero-length slot
    // that a reader mishandled would put both of those at the wrong offset - which is precisely
    // the failure the prefix exists to prevent, arrived at from the other direction.
    printf("state, length prefixes\n");
    {
        // Bind a channel row to a known controller FIRST, so the blob captured below has something
        // in it whose survival the fixtures can assert. It is taught here rather than inherited
        // from an earlier section: a fixture that depends on a neighbour's leftover state passes
        // or fails for reasons written down nowhere near it, and this one did exactly that until
        // the section it was inheriting from went away.
        {
            int od1 = kOd1;
        ...
        double trims[kChannelCount] = {};
        int32 slot = -1;
        check(readTrims(saved, trims, &slot), "the saved blob reads through to the slot");
        check(slot == 0, "this build writes a zero-length slot where the pedalboard used to sit");
        ...
        auto reloadAndCheck = [&](MemoryStream &blob, const char *what) {
            ...
            int32 s2 = -1;
            check(component->getState(&again) == kResultOk && readTrims(again, t, &s2),
                  "... and saves again");
            bool same = s2 == slot;
            for (int c = 0; c < kChannelCount && same; ++c)
                same = near(t[c], trims[c]);
            check(same, "... with every trim where it was and the slot still empty");
        };
        ...
        // window a host has on the controller: the parameters themselves.
        {
            ...
            check(mirrors, "and reads every trim at the same offset");
        }
        ...
        //
        // It is appended after that slot, so it is the one field whose offset depends
        ...
        // Its four channel bindings must survive intact - and, the part that costs a misread,
        // everything AFTER the table must land where it belongs.
        ...
        }
        {
            ...
            check(channel.sawChannelEcho && near(channel.channelEcho, channelValue(kOd1)),
                  "a version 5 project opens with its channel rows exactly as they were");
        }
