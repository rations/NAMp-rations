...
int main(int argc, char **argv)
{
    ...
    // REPORTED LATENCY MUST NOT MOVE WHEN THE CHANNEL IS SWITCHED, and this is the one place it
    // can be checked, because it is a question about the built bundle rather than about the DSP.
    //
    // The amp's latency is the resampler's alone and is fixed for the life of a setupProcessing,
    // so the correct answer is that nothing moves it. A channel switch is the strongest thing that
    // could: it rebinds models, runs a catch-up burst and crossfades, and it is exactly the kind
    // of change that would tempt a later version into reporting a different figure. A latency that
    // moved under it would make the host recompute its delay compensation mid-song, and some hosts
    // glitch when it does.
    //
    // BOTH DIRECTIONS ARE SWITCHED, and that is not thoroughness for its own sake. The ancestor of
    // this check moved a control one way only, and a deliberately broken build passed it, because
    // one direction is the state in which a conditional report and an unconditional one agree. The
    // bug worth catching is the figure changing on the way BACK.
    {
        ...
        auto latencyAfterSwitch = [&](double channelNorm) {
            Vst::ParameterChanges stomp;
            int32 qi = 0;
            if (Vst::IParamValueQueue *q = stomp.addParameterData(Rations::kChannelId, qi)) {
                int32 pi = 0;
                q->addPoint(0, channelNorm, pi);
            }
        ...
        // Away from Clean, then back to it. normFromChannel is what the parameter's own lane
        // carries, so this is the same value a host or a footswitch would write.
        const uint32 away = latencyAfterSwitch(Rations::normFromChannel(Rations::kChannelOd2));
        const uint32 back = latencyAfterSwitch(Rations::normFromChannel(Rations::kChannelClean));
        if (away != latency || back != latency) {
            fprintf(stderr,
                    "FAIL: reported latency moved under a channel switch - %u at rest, %u on OD2, "
                    "%u back on Clean. A host would recompute its delay compensation mid-song\n",
                    latency, away, back);
            return 1;
        }
        printf("latency        %u samples, unchanged by a channel switch in either direction\n",
               latency);
    }
